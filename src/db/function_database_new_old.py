...
from .region1 import region_1, region_2, region_3, region_4, region_7

# from create_cadics import create_cadics
Base = declarative_base()


class Comment_column(Base):
...
class Config(Base):
    ...
    status_config_device_details = relationship("Status_config_device_detail", back_populates="config")
    value_infs = relationship("Value_inf", back_populates="config")
...
class Information_project(Base):
    ...
    value_infs = relationship("Value_inf", back_populates="information_project")
...
class Lot(Base):
    ...
    status_lots = relationship("Status_lot", back_populates="lot")
...
class Project(Base):
    ...
    status_lots = relationship("Status_lot", back_populates="project")
...
class Project_has_config(Base):
    ...
    config = relationship("Config", back_populates="project_has_configs")
...
def querry_data(project_name):
    ...
    if project_name in all_project:
        result_querry_region1 = session.query(Information_project).all()
        print("result_querry_region1:", result_querry_region1)
        df_1 = region_1(result_querry_region1)
        # Aliased classes to prevent namespace collision if needed
        project_alias = aliased(Project)
        config_alias = aliased(Config)
        value_inf_alias = aliased(Value_inf)

        # ORM Query
        querry_2 = (
            session.query(
                value_inf_alias.parameter_name,
                project_alias.project_name,
                config_alias.config,
                value_inf_alias.value
            )
            .join(project_alias, project_alias.project_id == value_inf_alias.project_id)
            .join(config_alias, config_alias.config_id == value_inf_alias.config_id)
        )
        result_querry_region2 = querry_2.all()
        print("result_querry_region2: ", result_querry_region2)
        df_2 = region_2(result_querry_region2)
        merged_df = pd.merge(df_1, df_2, on='CADICS ID', how='inner')
        # print("merged_df.index: ", merged_df.index.max())

        result_querry_region3 = session.query(
            ...
            Config.config,
            Value_inf.value
        ).join(Project, Project.project_id == Value_inf.project_id
               ).join(Config, Config.config_id == Value_inf.config_id
                      ).join(Information_project, Information_project.parameter_name == Value_inf.parameter_name
                             ).filter(Project.project_name == 'WZ1J').all()
        print("result_querry_region3:", result_querry_region3)
        df_3 = region_3(result_querry_region3)

        # Truy vấn với ORM
        results_querry_region_4 = session.query(
            ...
            Device.device_group
        ).join(Device, Device.device_name == Device_details.device_name).all()
        index_df_1 = df_1.index.max()
        print("results_querry_region_4: ", results_querry_region_4)
        df_4 = region_4(results_querry_region_4, index_df_1)

        results_querry_region_7 = (
            session.query(
                Project.project_name,
                Config.config,
                Lot.lot_name,
                Project_has_config.optioncode_value,
                Status_lot.status_lot_optioncode
            )
            .join(Status_lot, Project.project_id == Status_lot.project_id)
            .join(Config, Status_lot.config_id == Config.config_id)
            .join(Project_has_config, (Status_lot.config_id == Project_has_config.config_id) & (
                    Status_lot.project_id == Project_has_config.project_id))
            .join(Lot, Status_lot.lot_id == Lot.lot_id)
            .filter(Project.project_name == 'WZ1J')
        ).all()
        ...
        df_end = pd.concat([merged_df, df_4, df_7], axis=0)
        session.close()
        return df_end
    else:
        session.close()
