...
import io
import pandas as pd
from ..db.function_database_new import get_gray_blue, update_data_new_online, delete_project, frame_empty, \
    update_data_new
    ...
    if get_data_syo_hyo("flag_view") == 1:
        ...
        with row_butt[3]:
            if get_data_syo_hyo("link_2") == "cadics_on_delete":
                if st.button("DELETE"):
                    df_after_deleted = delete_project(st.session_state.data_frame)
            ...
            if get_data_syo_hyo("link_syo") == "syo_hyo":
                ...
                if df is None or df.equals(frame_empty()):
                    ...
                    if st.session_state.data_frame.shape[1] != 4:
                        file_name = f"{project_name.upper()}.xlsx"
                    else:
                        file_name = "FORM.xlsx"
                    st.download_button(
                        label="Download File",
