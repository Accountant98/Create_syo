...
from src.app.merge_row_cadics import merge_row_cadics
import warnings

warnings.filterwarnings("ignore")
from src.app.check_zone import condition_zone_check
from src.app.check_option import check_option
...
def get_infor_car(df_spec):
    ...
    dict_optioncode = {}
    #working = os.path.dirname(__file__)
    link_form_cadic = "./form_out/form_cadics.csv"
        ...
        if len(address_zone[zone]) != 0:
            ...
            for index in col_index_zone_picked:
                ...
                if len(col_opt_pick) == 0:
                    ...
                    list_all_config, list_car_fix = list_config_all_(list_car, data_spec, list_except_config,
                                                                     dict_grade_zone, common_option)
        ...
        for key in dict_option.keys():
            if key != "グレード選択":
                ...
                if isinstance(value, str) == True and optioncode_config.find(value) != -1:
                    cmt = cmt.replace("'" + key + "': " + str(dict_option[key]),
                                      value + ":" + key + ": " + str(dict_option[key]))
