...
def rgb_to_ms_hls(red, green=None, blue=None):
  ...
  #print("blue: ", blue)
    ...
    if c_max == c_min:
        ...
        Rdelta = (((c_max - red) * (HLSMAX // 6)) + c_avg) // c_diff if c_diff != 0 else 0
        Gdelta = (((c_max - green) * (HLSMAX // 6)) + c_avg) // c_diff if c_diff != 0 else 0
        Bdelta = (((c_max - blue) * (HLSMAX // 6)) + c_avg) // c_diff if c_diff != 0 else 0
    ...
    for cell in row:
      #print(cell.coordinate)
        if cell.fill.start_color.type == 'theme':
            ...
            # print(f"Cell {cell.coordinate} has theme color: {color}")
            if color not in dict_return.keys():
                dict_return[color] = [cell.coordinate]
            else:
                dict_return[color].append(cell.coordinate)

            if color not in list_1:
                list_1.append(color)
        elif cell.fill.start_color.index != '00000000':
            color = cell.fill.start_color.rgb[2:]
            # print(f"Cell {cell.coordinate} has background color: {color}")
            if color not in dict_return.keys():
                dict_return[color] = [cell.coordinate]
            else:
                dict_return[color].append(cell.coordinate)
